...
![경량 세그멘테이션 — DeepLabV3+의 Encoder-Decoder 구조. Encoder가 특징을 압축하고, Atrous(팽창) Convolution이 파라미터를 늘리지 않고 넓은 시야를 확보하며, Decoder가 해상도를 복원해 픽셀별 라벨을 만든다](../assets/w10_p2_segmentation_04.png)
...
![엣지 가속 SDK 비교 — NVIDIA TensorRT는 Jetson(엣지 GPU)용으로 커널 튜닝·INT8 보정·엔진 빌드를 하고, Qualcomm SNPE는 스냅드래곤 NPU용으로 DLC 변환·유닛 선택·프로파일링을 제공한다](../assets/w10_p2_accel_sdk_05.png)

- **NVIDIA TensorRT** — Jetson 등 엔비디아 엣지 GPU를 위한 최적화 엔진이다. 레이어 융합·**커널 튜닝**과 **INT8 보정(Calibration)** 을 거쳐 **엔진 파일(.engine)** 을 빌드한다. 타깃 GPU 아키텍처(Ampere 등)에 맞는 최적 커널을 자동 선택한다(3주차 그래프 최적화의 실무판, 12주차로 심화).
...
최근 비전에서는 CNN 외에 **Vision Transformer(ViT)** 가 강력한 성능을 보인다. 그러나 ViT의 **Attention** 은 연산량이 크고 메모리를 많이 써 엣지 이식이 어렵다(11주차에서 Transformer의 온디바이스 병목을 심화).
