...
![논문 3대 트랙 — Track A(System: 가속기·컴파일러·런타임), Track B(Algorithm: 양자화·프루닝·증류·NAS), Track C(Application: 협업 추론·스마트홈·비전/언어 응용)](../assets/w14_p2_paper_tracks_04.png)

- **Track A · System** — 하드웨어·시스템 관점. 가속기 아키텍처, 컴파일러·런타임, Once-for-All의 확장 등(2·7·12주차). 대표 학회: ISCA, MobiCom.
- **Track B · Algorithm** — 모델·알고리즘 관점. 1~2비트 극단적 양자화(LLM PTQ, 학습 후 양자화), 프루닝·증류·NAS, 정확도 손실 원인 분석 등(4·5·6주차). 대표 학회: NeurIPS, ICML.
- **Track C · Application** — 응용·서비스 관점. 다중 오프라인 기기 간 협업 추론(Collaborative Edge Inference), 스마트홈(Matter) 생태계, 비전·언어·센서 응용 등(10·11·13주차). 대표 학회: CVPR, SenSys.
...
![비판적 논문 리뷰 프레임 — ① 주장(무엇을 개선?) ② 조건(어떤 하드웨어·데이터?) ③ 격차(시뮬레이션 vs 실제 보드) ④ 재현성(코드·설정 공개?). 자주 숨는 함정: FLOPs↓인데 지연 그대로, 압축률↑인데 속도 안 남, 특정 GPU에서만 나는 수치](../assets/w14_p2_critical_review_05.png)
...
이 강의에서 배운 통찰이 여기서 무기가 된다: "FLOPs가 줄어도 지연은 그대로일 수 있다(2주차)", "압축률이 높아도 속도는 안 날 수 있다(4주차)", "특정 GPU에서만 나는 수치일 수 있다(7·12주차)".
...
- **연합 학습 + 차분 프라이버시**(13주차)로 병원 간 데이터를 공유하지 않는 의료 AI.
- **Green AIoT + TinyML**(1.3 + 9주차)로 무전원 환경 모니터링 센서.
