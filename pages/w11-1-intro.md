# 11주차 1교시. 컴파일러 툴체인과 하드웨어 매핑

**강의 목표** — 고수준 딥러닝 모델이 하드웨어 전용 기계어로 번역되는 컴파일러 아키텍처를 이해하고, 가속기 백엔드의 구조를 파악한다.

지금까지 배운 최적화 모델(경량화·양자화)을 **실제 하드웨어에서 최대 성능으로 돌리는 것**이 11주차의 주제다. 3주차에서 본 그래프 최적화가 여기서 본격적인 **AI 컴파일러**로 심화되고, 2주차의 이기종 컴퓨팅이 실무 프로그래밍으로 종합된다.
...
![AI 컴파일러 아키텍처 — 모델 그래프가 High-level IR(하드웨어 독립적 그래프 최적화)을 거쳐 Low-level IR(하드웨어 종속적 루프 최적화)로 낮아지고, 타깃 기계어로 생성된다. TVM은 Tensor Expression으로 가속기별 최적 코드를 자동 생성한다](../assets/w11_p1_ai_compiler_01.png)
...
![Host-to-Device 병목과 비동기 실행 — 순차 실행은 전송 동안 가속기가 놀지만, 비동기(Async) 실행은 전송과 연산을 겹쳐 유휴 시간을 없앤다](../assets/w11_p1_host_device_02.png)
...
![Scratchpad/SRAM 재사용과 커스텀 오퍼레이터 — 가속기 내부 SRAM 버퍼에 텐서를 올려두고 재사용해 느린 DRAM 왕복을 줄이고, SDK가 지원하지 않는 특수 레이어는 C++/CUDA로 직접 구현한다](../assets/w11_p1_scratchpad_03.png)
