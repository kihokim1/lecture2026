...
한 학기 동안 배운 경량화(4~6주차)·설계 탐색(7주차)·하드웨어 가속(2·11주차)·측정(11주차)을 **하나의 실제 IoT 문제**에 통합해 적용하고, 그 결과를 **수치로 증명**하는 마무리 시간이다. 13주차 3교시에서 예고한 대로, 평가의 핵심은 "얼마나 그럴듯한가"가 아니라 "타깃 보드에서 실제로 얼마나 좋아졌는가"이다.
...
1. **문제·타깃 정의** — 어떤 IoT 문제를 어느 기기에 올리는가. 타깃 보드의 메모리·대역폭 제약(2·10주차)을 명시한다.
2. **적용 기법과 이유** — 프루닝·양자화·지식 증류·NAS·하드웨어 가속(4~7·11주차) 중 무엇을 왜 선택했는가.
3. **결과(수치)** — 정확도 방어율, 타깃 보드에서의 지연·메모리 개선율, 그리고 그것을 뒷받침하는 **프로파일링 근거**(11주차 방식, 필수).
...
| **하드웨어 프로파일링 증거 (필수)** | 15 | 11주차 방식으로 병목→개선을 **데이터로** 증명했는가 |
...
3. **프로파일링 원자료** — 11주차 방식의 연산자별 측정 결과(표/그래프)와 원본 로그.
