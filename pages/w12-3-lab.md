...
![프로파일링으로 병목 찾기 — 연산자별 소요 시간을 재서 가장 느린 연산(예: Conv2)이나 전송 오버헤드를 병목으로 식별하고 최적화 1순위로 삼는다](../assets/w12_p3_profiling_06.png)
...
> 교수님을 위한 Tip: 생성된 JSON을 브라우저 `chrome://tracing`(또는 최신 브라우저는 `ui.perfetto.dev`)에 넣으면 타임라인으로 시각화된다. 상용 프로파일러(Nsight 등)가 없어도 병목을 눈으로 보여줄 수 있어 실습 효과가 크다.
...
- 다음은 13주차 연합 학습·프라이버시로, 추론을 넘어 '기기에서의 학습·보안'으로 확장한다.
