...
![온디바이스 파인튜닝 LoRA — 전체 파인튜닝은 거대 가중치 W 전부를 갱신해 엣지 메모리로는 불가능하지만, LoRA는 원본 W를 고정(freeze)하고 작은 저차원 행렬(A·B)만 학습해 메모리를 크게 줄인다](../assets/w13_p2_lora_peft_04.png)

- **Memory-efficient Training** — 온디바이스 학습의 1차 장벽은 역전파의 메모리다. 이를 줄이는 여러 기법이 연구된다.
- **PEFT (Parameter-Efficient Fine-Tuning)** — 모델 전체가 아니라 **극히 일부 파라미터만** 학습하는 접근이다. 대표가 **LoRA(Low-Rank Adaptation)** 로, 원본 가중치 $W$ 는 **고정(freeze)** 한 채, 작은 저차원 행렬 $A, B$ 만 학습해 그 변화량을 더한다($W + BA$). 갱신할 파라미터가 극소라 엣지에서도 개인화 미세조정이 가능하다(11주차 On-device LoRA의 심화).
...
![프라이버시·보안 3종 — Differential Privacy(노이즈 추가로 역추적 방어), Secure Aggregation(암호화 상태로 합산해 서버도 개별 기여를 못 봄), TEE/TrustZone(신뢰 실행 환경에서 모델·연산을 하드웨어로 격리)](../assets/w13_p2_privacy_05.png)
