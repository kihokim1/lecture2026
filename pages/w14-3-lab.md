...
논문이 "고성능 GPU에서 초당 N토큰"을 주장할 때, **같은 모델을 엣지에 올리면 얼마나 나올까?** 11주차의 메모리-대역폭 근사로 직접 계산한다.

```python
def tokens_per_sec(model_gb, bw_gbps):
    # 11주차: 생성은 메모리-바운드 → 속도 ≈ 대역폭 / 모델크기 (이론 상한)
    return bw_gbps / model_gb

model_gb = 3.5                      # 4비트로 양자화한 7B 모델(11주차)
...
2. 원인은 성능이 곧 **메모리 대역폭**에 종속되기 때문이다(2·11주차). 알고리즘이 아니라 **기기**가 상한을 정한다.
...
![1~14주차 기술 종합 로드맵 — 기초·이론(1~3) → 경량화·설계(4~7) → 응용·실무(9~13) → 종합(14~15). 관통하는 통찰: 성능을 좌우하는 것은 연산이 아니라 데이터 이동이며, 하드웨어가 활용할 수 있어야 실제로 빨라진다](../assets/w14_p3_roadmap_06.png)

15주차 최종 캡스톤 발표를 앞두고 평가지표를 다시 정리한다.

- **정확도(Accuracy) 방어율** — 경량화 후에도 정확도를 얼마나 지켰는가.
- **타깃 보드에서의 지연(Latency) 개선율** — 실제 기기에서 얼마나 빨라졌는가.
- **하드웨어 프로파일링 결과 제시** — 12주차 방식으로 병목과 개선을 데이터로 증명(필수).
...
- 한 학기를 종합해, 15주차 기말 캡스톤(정확도·지연·프로파일링)으로 나아간다.
