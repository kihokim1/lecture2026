...
![TensorRT 엔진 빌드 파이프라인 — 모델 파싱 → 그래프 최적화(레이어 융합·정밀도 선택) → Kernel Tuning(GPU별 최적 커널) → INT8 Calibration → .engine 파일. 빌드는 한 번, 이후 엔진을 로드해 반복 추론](../assets/w12_p2_tensorrt_04.png)
...
![모바일·임베디드 NPU SDK 비교 — TensorRT(NVIDIA Jetson), SNPE(Qualcomm 스냅드래곤 NPU, DLC 변환·유닛 선택), ARM NN/CMSIS-NN(Cortex·Mali·Ethos 통합 스택)](../assets/w12_p2_npu_sdk_05.png)
...
- **ARM NN / CMSIS-NN** — ARM 기반 임베디드 리눅스·MCU(마이크로컨트롤러) 환경에서 Cortex CPU·Mali GPU·Ethos NPU를 아우르는 **추론 SDK(라이브러리)**다. CMSIS-NN은 9주차 TinyML에서 이미 다뤘다.
...
![프로파일링 — 연산자별 소요 시간을 재서 가장 느린 연산(예: Conv2)이나 전송 오버헤드(H2D)를 병목으로 식별하고, 최적화 1순위로 삼는다](../assets/w12_p3_profiling_06.png)
...
15주차 최종 발표를 앞두고, 9~11주차에 선택한 도메인 모델(비전/언어)이 12주차의 가속 SDK(TensorRT 등)를 거쳐 **실제 타깃 보드에서 기동되는지** 중간 점검한다. 평가지표는 정확도 방어율, 타깃 보드에서의 지연(Latency) 개선율, 프로파일링 결과 제시다.
