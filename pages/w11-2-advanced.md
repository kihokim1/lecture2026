# 11주차 2교시. On-Device LLM 최적화와 런타임
...
![극단적 양자화 — 7B 모델 가중치는 FP32에서 약 28GB이지만, FP16≈14GB, INT8≈7GB, INT4≈3.5GB로 줄어야 소비자 기기에 적재 가능하다. AWQ·GPTQ가 정확도 손실을 최소화한다](../assets/w11_p2_quant_4bit_04.png)
...
![Speculative Decoding — 작은 Draft 모델이 여러 토큰을 먼저 제안하고, 큰 모델이 이를 한 번에 검증한다. 초안이 자주 맞으면 큰 모델 호출이 줄어 속도가 오른다. 정확도는 큰 모델이 최종 결정하므로 그대로](../assets/w11_p2_speculative_05.png)
...
- **On-device LoRA** — **LoRA(Low-Rank Adaptation)** 는 원본 가중치는 고정한 채 극히 일부의 저차원 행렬만 학습하는 파인튜닝 기법이다. 이를 엣지에 적용하면 기기 위에서 개인화 미세조정이 가능해진다(13주차 On-device 파인튜닝에서 심화).
