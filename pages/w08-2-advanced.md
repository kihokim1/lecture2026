...
![Duty Cycling — 평소 Deep Sleep으로 극소 전력을 유지하다가, 추론이 필요할 때만 짧게 Active 상태로 전환한다. 평균 전력 = 활성 시간 비율 × 활성 전력](../assets/w08_p2_duty_cycling_04.png)
...
![표준 Conv vs Depthwise Separable — 공간과 채널을 한꺼번에 처리하는 표준 Conv를, 채널별 공간 합성곱(Depthwise)과 1×1 채널 혼합(Pointwise) 두 단계로 분리해 연산량을 크게 줄인다](../assets/w08_p2_depthwise_05.png)
...
이렇게 분리하면 연산량이 대략 "$\frac{1}{\text{출력 채널 수}} + \frac{1}{\text{커널 면적}}$" 수준으로 줄어든다. MobileNet 계열(9주차 비전에서 재등장)이 이 구조로 모바일·엣지에서 실시간성을 확보한 핵심이다.
...
- **Battery-less / Intermittent Computing** — 배터리 없이 **태양광·진동·열 등에서 에너지를 수확(Energy Harvesting)** 해 작동하는 AI 시스템 연구다. 전력 공급이 **간헐적(intermittent)** 이라, 전원이 끊겨도 연산 상태를 보존했다가 이어서 계산하는 기법(비휘발성 메모리 활용 등)이 함께 연구된다(13주차에서 심화).
