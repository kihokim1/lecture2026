# 13주차 1교시. 온디바이스AI 최신 연구 패러다임

**강의 목표** — 최근 1~2년간 학계·산업계에서 가장 주목받는 온디바이스AI의 한계 돌파 기술을 조망한다.
...
10주차에서 본 On-Device LLM을 더 밀어붙이는 연구들이다.

![패러다임 1 — 이기종 코어 융합 디코딩(Speculative Decoding을 넘어 NPU와 GPU를 동시 활용)과 Activation Sparsity(입력에 따라 활성 뉴런 자체를 제한해 연산량을 동적으로 줄임)](../assets/w13_p1_paradigm_llm_01.png)

- **이기종 코어 융합 디코딩** — 10주차의 Speculative Decoding을 넘어, **NPU와 GPU 같은 이기종 코어를 동시에 활용**해 동적으로 토큰을 생성하는 연구다. 작은 코어가 초안을, 큰 코어가 검증을 맡는 식으로 협업해 지연을 더 줄인다.
...
![패러다임 2 — IMU·진동·PPG 등 IoT 센서 데이터로 사전학습한 트랜스포머(Time-Series Foundation Model)를 스마트 팩토리·헬스케어 등에 크로스 도메인 전이학습으로 재활용한다](../assets/w13_p1_timeseries_fm_02.png)
...
다만 이 대형 모델을 다시 **엣지에 올리려면** 1~12주차의 경량화·양자화·가속이 그대로 필요하다. 즉 최신 연구도 이 강의의 기초 위에 서 있다.
...
8주차 TinyML의 초저전력을 극단(무전원)까지 밀어붙이는 연구다.

![패러다임 3 — 태양광·진동·열에서 에너지를 수확(Energy Harvesting)하고, 전력이 간헐적(Intermittent)이라 비휘발성 메모리(NVM)로 연산 상태를 보존했다 이어서 계산하는 Net-Zero AIoT](../assets/w13_p1_green_aiot_03.png)
