# 13주차 1교시. 연합 학습의 메커니즘과 통신 최적화

**강의 목표** — 중앙 집중식 학습의 프라이버시 한계를 극복하는 연합 학습의 분산 알고리즘을 이해한다.

지금까지의 온디바이스 AI는 대부분 **추론(Inference)** 에 집중했다. 13주차는 한 걸음 더 나아가, **기기 위에서 학습(Training)** 하되 프라이버시를 지키는 **연합 학습(Federated Learning, FL)** 을 다룬다. 온디바이스 AI의 종착지에 해당하는 주제다.
...
![중앙 집중식 vs 연합 학습 — 중앙 집중은 원본 데이터를 서버로 업로드해 프라이버시 위험이 있고, 연합 학습은 데이터를 기기에 남긴 채 학습 결과(가중치)만 주고받는다](../assets/w13_p1_centralized_vs_fl_01.png)
...
![FedAvg 순환 — ① 서버가 글로벌 모델을 각 기기에 배포 → ② 각 기기가 로컬 데이터로 학습(Local SGD) → ③ 학습된 가중치를 서버로 업로드 → ④ 서버가 데이터 크기로 가중 평균해 글로벌 모델을 갱신](../assets/w13_p1_fedavg_02.png)
...
![Non-IID 데이터 문제 — 기기마다 데이터 분포가 편향되어(고양이만/자동차만/새만) 각 로컬 모델이 다른 방향으로 학습되면, 평균 시 성능이 저하되거나 발산할 수 있다](../assets/w13_p1_noniid_03.png)
