# 09주차 1교시. TinyML의 정의와 기술적 제약

**강의 목표** — RAM이 수백 KB에 불과한 환경에서 AI를 구동하기 위한 '극단적 경량화'의 필요성과 하드웨어 제약을 이해한다.

8주차 중간고사로 전반부 이론(하드웨어·프레임워크·경량화·NAS)을 마쳤다. 9주차부터는 그 기법들이 실제로 쓰이는 **응용 영역**으로 들어간다. 첫 주제는 자원 제약의 극단, **TinyML**이다.
...
![컴퓨팅 스펙트럼 — 클라우드에서 엣지 GPU, 모바일을 거쳐 MCU(TinyML)로 갈수록 RAM과 전력이 급감한다. TinyML은 수백 KB RAM·수 mW 전력 환경](../assets/w09_p1_tinyml_spectrum_01.png)
...
![MCU의 자원 제약 — RAM 수십~수백 KB, Flash 수백 KB~수 MB, FPU가 없거나 느려 정수 연산에 의존, 전력은 mW 수준. RAM이 Peak Memory를 못 넘겨야 시스템이 죽지 않는다](../assets/w09_p1_mcu_constraints_02.png)
...
![TinyML 소프트웨어 스택 — 양자화된 .tflite 모델을 TFLite for Microcontrollers(TFLM)가 실행하고, CMSIS-NN이 Cortex-M의 SIMD로 연산을 가속한다](../assets/w09_p1_sw_stack_03.png)
