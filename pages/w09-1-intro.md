# 09주차 1교시. 경량 비전 아키텍처와 객체 탐지

**강의 목표** — 모바일·엣지 환경에 최적화된 CNN 구조를 분석하고, 실시간성 확보를 위한 객체 탐지 알고리즘의 최적화 방안을 이해한다.

8주차까지 배운 경량화·저전력 기법이 실제로 빛을 발하는 첫 응용 분야가 **컴퓨터 비전(Computer Vision)** 이다. 서버급 GPU에서 돌던 무거운 비전 모델을, 성능 저하를 최소화하며 엣지의 NPU/GPU에 올리는 기법을 다룬다.
...
![모바일 백본의 진화 — MobileNet v1(Depthwise Separable) → v2(Inverted Residual) → v3(NAS+SE) 그리고 EfficientNet-Edge(복합 스케일링). 목표는 FLOPs가 아니라 실제 엣지 지연 단축](../assets/w09_p1_backbones_01.png)

- **MobileNet 시리즈** — 엣지 비전의 대표 백본이다.
  - **v1**: 8주차에서 배운 **Depthwise Separable Convolution**으로 연산량을 대폭 줄였다.
...
![실시간 객체 탐지 — 경계 상자+클래스+신뢰도를 예측한다. YOLO는 한 번에 예측하는 one-stage로 빠르고, SSD+MobileNet은 다중 스케일 박스로 속도-정확도를 조율한다](../assets/w09_p1_detection_02.png)
...
![NMS 후처리 — 같은 객체에 겹쳐 나온 여러 상자 중 신뢰도가 가장 높은 하나만 남기고 나머지를 억제한다. NMS는 순차 비교라 엣지에서 병목이 되기 쉬워, Anchor-free 방식으로 후처리를 줄이는 흐름이 있다](../assets/w09_p1_nms_03.png)
