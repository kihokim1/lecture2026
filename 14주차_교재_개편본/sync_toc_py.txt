...
위키독스는 TOC.md 를 파싱해 **페이지의 계층 구조와 페이지 제목**을 만든다.
- 각 항목의 대괄호 안 문자열이 **페이지 제목**이 된다. (본문 H1 이 아니라 이 값이다)
- **책 제목(최상위)은 TOC.md 가 아니라 위키독스의 `책 제목` 설정에서 온다.**
  따라서 최상위가 잘못되었으면 재동기화로는 못 고치고 위키독스에서 직접 고쳐야 한다.
- push 하면 웹훅으로 자동 동기화되고, 책 편집 화면의 깃허브 탭에서 `지금 동기화` 로도 실행할 수 있다.
...
def main() -> int:
    ...
    if lines[0].strip() != BOOK_TITLE:
        problems.append(f"[경고] TOC.md 첫 줄이 책 제목과 다르다: {lines[0]!r}\n"
                        f"       기대값 {BOOK_TITLE!r}. (최상위 제목 자체는 위키독스 설정에서 온다)")
